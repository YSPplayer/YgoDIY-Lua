--奥利哈刚的元素 （ZCG）
function c98710526.initial_effect(c)
	--Activate
	...
	e2:SetValue(c98710526.costchange)
	c:RegisterEffect(e2)
--
...
--recover
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c98710526.target)
	e3:SetOperation(c98710526.activate)
	c:RegisterEffect(e3)
 --spsummon
	...
	e9:SetCondition(c98710526.spcon2)
	e9:SetTarget(c98710526.rettg)
	e9:SetOperation(c98710526.retop)
	c:RegisterEffect(e9)

end
function c98710526.costchange(e,re,rp,val)
 if re then
		return 0
	else
		return val
...
function c98710526.spcon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_ONFIELD)
end
function c98710526.rettg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c98710526.retop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		Duel.SendtoHand(e:GetHandler(),nil,REASON_EFFECT)
	end
end
function c98710526.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if #g1>0 and #g2>2 then
		local opt=Duel.SelectOption(1-tp,aux.Stringid(98710526,1),aux.Stringid(c98710526,2))
		e:SetLabel(opt)
		if opt==0 then
...
function c98710526.activate(e,tp,eg,ep,ev,re,r,rp)
	local g1=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_ONFIELD,nil)
	local g2=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_DECK,nil)
	if #g1>0 and #g2>2 then
