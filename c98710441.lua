--奥利哈刚之气 （ZCG）
function c98710441.initial_effect(c)
		--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(98710441,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCondition(c98710441.condition)
	e1:SetOperation(c98710441.moop)
	c:RegisterEffect(e1)
		--Activate
	...
	e2:SetCondition(c98710441.condition)
	e2:SetTarget(c98710441.motg)
	e2:SetOperation(c98710441.moop2)
	c:RegisterEffect(e2)
	--selfdes
	...
	e4:SetCondition(c98710441.sdcon2)
	e4:SetOperation(c98710441.sdop)
	c:RegisterEffect(e4)
	...
	e3:SetCondition(c98710441.atkcon)
	e3:SetCost(c98710441.cost)
	e3:SetTarget(c98710441.atktg)
	e3:SetOperation(c98710441.atkop)
	c:RegisterEffect(e3)
   --
	...
	e5:SetCountLimit(1) 
	e5:SetCondition(c98710441.reccon)
	e5:SetTarget(c98710441.target)
	e5:SetOperation(c98710441.operation)
	c:RegisterEffect(e5)
	--indes
...
function c98710441.reccon(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer()
end
function c98710441.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c98710441.operation(e,tp,eg,ep,ev,re,r,rp)
	local rec=Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)*500
	local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
	Duel.Recover(p,rec,REASON_EFFECT)
end
function c98710441.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,nil,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,nil,1,1,nil)
	Duel.Release(g,REASON_COST)
end
function c98710441.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return tp~=Duel.GetTurnPlayer()
end
function c98710441.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c98710441.atkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetAttacker()
	if tc:IsRelateToEffect(e) and tc:IsAttackable() and Duel.NegateAttack() then
		Duel.Destroy(tc,REASON_EFFECT)
	end
end
function c98710441.cfilter(c)
	return c:IsFaceup() and c:IsCode(98710440)
end
function c98710441.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c98710441.cfilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function c98710441.sdcon2(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(98710441)==0
end
function c98710441.sdop(e,tp,eg,ep,ev,re,r,rp) 
	e:GetHandler():CopyEffect(98710440,RESET_EVENT+0x1fe0000)
	e:GetHandler():RegisterFlagEffect(98710441,RESET_EVENT+0x1fe0000,0,1)
end
function c98710441.motg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c98710441.moop(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	Duel.MoveToField(c,tp,tp,LOCATION_FZONE,POS_FACEUP,true)
end
function c98710441.moop2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
