--奥利哈刚的诅咒 （ZCG）
function c98710509.initial_effect(c)
	local e0=Effect.CreateEffect(c)
	e0:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e0:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e0:SetCode(EVENT_PREDRAW)
	e0:SetCountLimit(1,98710509+EFFECT_COUNT_CODE_DUEL)
	e0:SetRange(LOCATION_DECK)
	e0:SetCondition(c98710509.mocon)
	e0:SetOperation(c98710509.moop)
	c:RegisterEffect(e0)
--
	...
	e1:SetCountLimit(1,98710509+EFFECT_COUNT_CODE_DUEL)
	e1:SetRange(LOCATION_DECK)
	e1:SetCondition(c98710509.mocon2)
	e1:SetOperation(c98710509.moop2)
	c:RegisterEffect(e1)
end
function c98710509.mocon(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer()
end
function c98710509.moop(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c98710509.mocon2(e,tp,eg,ep,ev,re,r,rp)
	return 1-tp==Duel.GetTurnPlayer()
end
function c98710509.moop2(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_SZONE,0)>0 then
	Duel.MoveToField(c,tp,tp,LOCATION_SZONE,POS_FACEUP,true)
	Duel.BreakEffect()
