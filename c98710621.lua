--太阳神之绿化龙 （ZCG）
function c98710621.initial_effect(c)
	  --draw
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(98710621,0))
	e1:SetCategory(CATEGORY_DRAW+CATEGORY_ATKCHANGE)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetTarget(c98710621.target)
	e1:SetOperation(c98710621.activate)
	c:RegisterEffect(e1)
	...
	e52:SetOperation(c98710621.disop2)
	...
	e53:SetTarget(c98710621.distg2)
	...
	e54:SetTarget(c98710621.distg2)
	c:RegisterEffect(e54)
end
-------------------------------------------------------------------------
function c98710621.disop2(e,tp,eg,ep,ev,re,r,rp)
...
function c98710621.distg2(e,c)
	return c:GetCardTargetCount()>0 and c:IsType(TYPE_TRAP)
		and c:GetCardTarget():IsContains(e:GetHandler())
end
-------------------------------------------------------------------------
function c98710621.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(5)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,5)
end
function c98710621.activate(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	if Duel.Draw(p,d,REASON_EFFECT)==0 then return end
	Duel.BreakEffect()
	local g=Duel.GetOperatedGroup()
	Duel.ConfirmCards(1-tp,g)
	local dg=g:Filter(c98710621.filter,nil)
