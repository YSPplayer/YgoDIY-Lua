--奥利哈刚 拉菲鲁 （ZCG）
function c98710419.initial_effect(c)
			  --special summon
	...
	e1:SetCondition(c98710419.spcon3)
	c:RegisterEffect(e1)
	--special summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(98710419,0))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_REMOVED)
	e2:SetCondition(c98710419.spcon3)
	e2:SetTarget(c98710419.sptg3)
	e2:SetOperation(c98710419.spop3)
	c:RegisterEffect(e2)
--SpecialSummon
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(98710419,1))
	e4:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCountLimit(1)
	e4:SetTarget(c98710419.target)
	e4:SetOperation(c98710419.activate)
	c:RegisterEffect(e4)
end
function c98710419.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c98710419.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c98710419.sptg3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c98710419.spop3(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
